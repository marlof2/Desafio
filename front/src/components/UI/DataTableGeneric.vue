<template>
  <div>
    ...
    >
      ...
      <template  v-for="colunm in colunmCustom"  v-slot:[`item.${colunm}`]="{item}">
      ...
      >
        <div :key="customItem.id">
          ...
          <div
            v-if="customItem.filter === 'cep' && item[customItem.name]"
          >
    ...
    <span v-if="showPaginate && dataProp.length">
      ...
      <div
        v-if="showSelectPagination"
        class="mt-2 d-flex flex-row align-items-flex-start justify-center"
      >
        <SelectAutocomplete
          v-model="page"
